import { View, Text, StyleSheet, Button, Dimensions, TouchableOpacity, ScrollView, FlatList, Modal } from "react-native";
import { React, useEffect, useState } from "react"
import DropDownPicker from 'react-native-dropdown-picker';
import FontAwesome from '@expo/vector-icons/FontAwesome';
import Toast from "react-native-root-toast";
import Hr from "../components/Hr";
import { doc, setDoc, getDoc, updateDoc, arrayUnion, deleteDoc, arrayRemove } from "firebase/firestore";
...
const Treino = () => {
  ...
  const [items, setItems] = useState([
    ...
    { label: 'Ausência de Pés ou Dedos dos Pés', value: 'Ausência de Pés ou Dedos dos Pés' },
    ...
    { label: 'Transtorno do Déficit de Atenção e Hiperatividade (TDAH)', value: 'Transtorno do Déficit de Atenção e Hiperatividade (TDAH)' },
    { label: 'Dificuldades na Coordenação Motora', value: 'Dificuldades na Coordenação Motora' },
  ...
  async function setDeficiencia(value) {
    ...
    if (actualDefs.includes(value)) return Toast.show("Deficiência já adicionada!");
    ...
    <View style={styles.container}>
      ...
      <View style={styles.minhaDeficienciaView}>
        <View style={styles.minhaDeficienciaHeader}>
          ...
          <TouchableOpacity onPress={() => setModalAddDefVisible(!modalAddDefVisible)}>
          ...
          <FlatList
              ...
              renderItem={({ item }) => (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                  ...
                  <FontAwesome name="remove" size={25} color="red" onPress={() => deletarDef({ item })} />
                </View>
              )}
            />
          </View>
      ...
      >
        <View style={styles.containerModalAddDeficiencia}>
          <View style={styles.modalAddDeficiencia}>
            <Text style={styles.deficienciaText}>Selecione suas deficiências!</Text>
            <DropDownPicker
              open={open}
              value={value}
              items={items}
            ...
            />
              ...
              <TouchableOpacity style={styles.BtnAdd} onPress={() => {
                    ...
                    if (value != undefined) setModalAddDefVisible(!modalAddDefVisible)
                  }}
                >
